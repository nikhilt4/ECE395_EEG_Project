...
=====================================================
Run this before the games to validate your EEG pipeline.
Connects to STM32 on COM5, waits for READY, sends START,
collects baseline, then shows live ERD ratio and LEFT/RIGHT
classifications in real time.

KEYBOARD:
    C     — clear trial log
    S     — print stats to terminal
    R     — send RECALIBRATE to STM32 (redo baseline)
    ESC   — quit
"""
...
_serial_ref  = [None]   # holds the serial object so R key can send RECALIBRATE


# ─────────────────────────────────────────────
#  SERIAL THREAD — mirrors lane_dodge exactly
# ─────────────────────────────────────────────
def eeg_serial_thread():
    try:
        ...
        print("[EEG] Running without hardware — use keyboard to inject test events")
        return
        ...
        while not _serial_stop.is_set():
            try:
                ...
                if line.startswith("I,"):
                    msg = line[2:]
                    print(f"[STM] {msg}")
                    # Pass all I, messages to the monitor for display
                    eeg_queue.put(("info", msg))
                    if "BASELINE_DONE" in msg:
                        eeg_queue.put(("baseline_done", None))
                ...
                if line.startswith("BP,"):
                    ...
                    # Convert ratio (-1 to +1) to left/right probabilities
                    # ratio > 0 = LEFT, ratio < 0 = RIGHT
                    # Map to 0-1 probability space for the monitor bars
                    left_prob  = max(0.0, min(1.0,  ratio + 0.5))
                    right_prob = max(0.0, min(1.0, -ratio + 0.5))

                    eeg_prob_queue.put((left_prob, right_prob, ratio, cls))
                    print(f"[EEG] ratio={ratio:+.4f}  cls={cls} "
                          f"({'LEFT' if cls==1 else 'RIGHT' if cls==2 else 'REST'})")
...
class NeurofeedbackMonitor:
    ...
    def reset(self):
        ...
        # Baseline state — mirrors firmware
        self.eeg_ready         = False
        self.baseline_progress = "waiting..."
        self.last_info         = ""

    def _drain(self):
        # Drain info/baseline messages
    ...
    def draw(self):
        ...
        # ── Calibration overlay — shown until BASELINE_DONE ──
        if not self.eeg_ready:
            ...
            return   # don't draw rest of UI until calibrated
        ...
        # Centre: current label + raw ratio
        cx  = W // 2
        ...
        # Raw ERD ratio display — this is the key diagnostic number
        ...
        for i, r in enumerate(self.hist_ratio):
            ...
            # map -1..+1 to bottom..top of panel
        ...
        # Last STM info message (wraps on second line)
...
def main():
    ...
    monitor = NeurofeedbackMonitor(screen, fonts)   # create monitor FIRST

    # Start serial thread AFTER display is up
    # If COM5 is not connected, monitor still opens in keyboard-only mode
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r:
                    # Send RECALIBRATE to STM32 and reset monitor state
                    ser = _serial_ref[0]
                    if ser and ser.is_open:
                        ser.write(b"RECALIBRATE\n")
                        ser.flush()
                        print("[EEG] RECALIBRATE sent")
                ...
                if event.key == pygame.K_LEFT:
                    eeg_prob_queue.put((0.8, 0.2, 0.6, 1))   # fake LEFT
                    
                if event.key == pygame.K_RIGHT:
                    eeg_prob_queue.put((0.2, 0.8, -0.6, 2))  # fake RIGHT
                    
                if event.key == pygame.K_SPACE:
                    eeg_prob_queue.put((0.5, 0.5, 0.0, 0))   # fake REST
                    
                # Also force baseline done for testing without hardware
